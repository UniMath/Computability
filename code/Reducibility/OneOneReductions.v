...
Section ManyManyOneOneCharacterisation.
  ...
  Lemma predcylinder1 {X Z : UU} (p : X → hProp) (setX : isaset X) (setZ : isaset Z) (z : Z) : (p ⪯₁ (predcylinder p Z)).
  Proof.
    apply hinhpr.
    use make_oneonereduction.
    - use make_incl.
      + intros x. exact (x,, z).
      + apply isinclbetweensets. exact setX. apply (isaset_dirprod setX setZ).
        intros x x' eq. 
        set (q := maponpaths dirprod_pr1 eq). simpl in q. exact q. 
  ...
  Lemma manyone_oneone1 {X Y : UU} (Z : UU) (setY : isaset Y) (setX : isaset X) (setZ : isaset Z) (p : X → hProp) (q : Y → hProp) (g : incl (Y) Z) : (ismanyonereducible q (predcylinder p Z)) → (q ⪯₁ (predcylinder p Z)).
  Proof.
    intros ismo.
    use squash_to_prop.
    ...
    - use make_incl.
      ...
      + apply isinclbetweensets. exact setY. apply (isaset_dirprod setX setZ).
        intros y y' eq. 
        set (eqq := maponpaths dirprod_pr2 eq). simpl in eqq.
        destruct g as [g isinclg]. simpl in *.
        apply (invmaponpathsincl g isinclg), eqq.
  ...
  Lemma manymany_oneone2 {X Y Z : UU} (setX : isaset X) (setY : isaset Y) (setZ : isaset Z) (z : Z) (f : incl (Y × Z) Z) (p : X → hProp) (q : Y → hProp) : (ismanyonereducible q p) <-> ((predcylinder q Z) ⪯₁ (predcylinder p Z)).
  Proof. 
    split.
    - intros impl1.
      apply manyone_oneone1. 
        apply (isaset_dirprod setY setZ). exact setX. exact setZ.  
      apply f.
